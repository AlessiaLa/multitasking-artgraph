...
def fine_tune(model, graph, train_loader, validation_loader,
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for phase in ['train', 'val']:
            ...
            for image_features, (style_labels, genre_labels, emotion_labels) in tqdm(data_loader):
...
def fine_tune_single_task(model, graph, train_loader, validation_loader,
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for phase in ['train', 'val']:
            ...
            for image_features, labels in tqdm(data_loader):
                image_features = image_features.to(device, non_blocking=True)
...
def test_single_task(model, data_loader, graph, update_graph = False, device=torch.device('cuda:0')): #cuda:0
    tot_pred = None
    tot_lab = []
...
def test(model, data_loader, graph, update_graph=False, device=torch.device('cuda:0')): #cuda:0
